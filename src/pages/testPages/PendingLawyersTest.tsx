...
import {
  getPendingLawyers,
  getPendingLawyerDetail,
  approveLawyerRegister,
...
export const PendingLawyersPage = (): React.JSX.Element => {
  ...
  const [pendingLawyers, setPendingLawyers] =
    React.useState<PageResponse<PendingLawyers>>();
  const [detailedPendingLawyer, setDetailedPendingLawyer] =
    React.useState<DetailedPendingLawyer>();
  const [selectedLawyerId, setSelectedLawyerId] = React.useState<number>(0);

  const fetchPendingLawyers = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    try {
  ...
  const fetchDetailedPendingLawyer = async (
  ...
  return (
    <div>
      <div className="flex flex-col gap-10px">
        <h1>변호사 승인 대기 조회</h1>
        <form onSubmit={fetchPendingLawyers}>
          <input
            className="border-2 border-lightGreen"
            type={"number"}
            value={page}
            onChange={(e) => setPage(Number(e.target.value))}
            placeholder="현재 페이지 입력"
          />
          <input
            className="border-2 border-lightGreen"
            type={"number"}
            ...
            placeholder="페이지당 조회할 개수 입력"
          />
          <button className="bg-lightGreen rounded-10px" type={"submit"}>
            조회하기
          </button>
        </form>
        {pendingLawyers && (
          <div className="flex flex-col gap-10px border-2 border- rounded-10px">
      ...
      <div>
        ...
        <form onSubmit={fetchDetailedPendingLawyer}>
          <div>
        ...
        {detailedPendingLawyer && (
          <>
            <div className="border-2 border-lightGreen rounded-10px flex flex-col">
              ...
              <img
                src={`${IMAGE_URL}${detailedPendingLawyer.licenseImageInfo.path}`}
                alt="자격증 이미지"
            ...
            <div>
              {detailedPendingLawyer.approvalStatus ===
                ApprovalStatus.WAITING && (
                <>
                  <button
                    className="bg-lightGreen rounded-10px"
                    onClick={() =>
                      handleApproveLawyer(detailedPendingLawyer.lawyerId, true)
                    }
                  >
                    승인하기
                  </button>
                  <button
                    className="bg-red-500 rounded-10px"
                    onClick={() =>
                      handleApproveLawyer(detailedPendingLawyer.lawyerId, false)
                    }
                  >
                    거절하기
                  </button>
