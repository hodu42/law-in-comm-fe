...
export const RecentQuestionsContent = ({isAnonymous, authorId, authorName, questionId, legalSpeciality, title, content, createdAt, firstOccurrenceDate, viewCount, reportCount}:Question):React.JSX.Element => {
    return (
        <Link to={`/question/${questionId}`} className="block w-full min-h-[250px] pc:w-[848px] pc:h-[396px] bg-white rounded-[10px] p-[20px] pc:p-[30px] border-2 border-[#5C6E56] transition-all relative">
            <div className="flex flex-col gap-[15px] h-full">
                ...
                <h3 className="text-[1rem] pc:text-[1.31rem] font-medium break-words line-clamp-1">{title}</h3>
            ...
            <div className="mt-4 absolute bottom-1 pc:bottom-0 right-0 p-[15px] pc:p-[30px] flex justify-end items-center gap-4 text-[0.8rem] pc:text-[1rem] text-[#B4B4B4]">
                ...
                <div className="flex items-center gap-1">
                    ...
                    <svg className="w-[13px] h-[14px] pc:w-[16px] pc:h-[17px] flex-shrink-0 translate-y-[1px]" viewBox="0 0 16 17" fill="none">
