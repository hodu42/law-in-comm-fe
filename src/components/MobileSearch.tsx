...
export const MobileSearch = ():React.JSX.Element => {
    ...
    const {setKeyword: setURLKeyword} = useAppDispatch();
    ...
    return (
        <form className="flex pc:hidden w-[90%] items-center mt-[112px]" action="" method="GET" onSubmit={handleSubmit}>
            ...
            <input className=" flex w-[100%] h-11 pr-3.5 pl-[61px] rounded-10px placeholder:text-[#E2E4E5] border-2 border-transparent focus:border-[#9CB395] focus:outline-none" type="search" value={keyword} onChange={(e) => setKeyword(e.target.value)} placeholder="검색어를 입력해주세요."/>
        </form>
    )
}