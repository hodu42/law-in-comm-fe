    ...
    return (
        ...
        <nav className="hidden absolute mobile:flex items-center w-full h-[72px] gap-[50px]">
            {headerPaths.map((path) => (
                <a href={path.link} className="text-[#CECFD3] text-[24px] font-bold py-4 h-full hover:text-black hover:border-b-black hover:border-b-2" aria-current={path.link === window.location.pathname ? 'page' : undefined}>
                    {path.title}
                </a>
            ))}
        </nav>
