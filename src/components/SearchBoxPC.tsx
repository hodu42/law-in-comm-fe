...
export const SearchBoxPC = ():React.JSX.Element => {
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        goToQuestionList(keyword, currentCategory, '0');
    }
    return (
        <form className="z-0 hidden pc:flex w-[57%] items-center" method="GET" onSubmit={handleSubmit}>
            ...
            <input className="focus:outline-none flex w-[100%] h-11 pr-3.5 pl-[61px] border-[2px] rounded-10px border-lightGreen placeholder:text-[#E2E4E5]" value={keyword} type="search" placeholder="검색어를 입력해주세요." onChange={(e) => setKeyword(e.target.value)}/>
        </form>
    )
}