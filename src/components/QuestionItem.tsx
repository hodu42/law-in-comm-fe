...
export const QuestionItem: React.FC<{ question: Question; answer: Answer | null }> = ({ question, answer }) => {
  return (
    <Link to={`/question/${question.questionId}`} className="bg-white rounded-[10px] shadow-sm mb-10 pc:p-[30px] border-2 border-transparent hover:border-[#9CB395]">
      <div className="flex flex-col justify-between gap-[15px]">
        <div className="flex items-center text-sm px-2">
          <span className="mr-4 text-[1.18rem] text-[#848484]">{LegalSpecialityLabels[question.legalSpeciality]}</span>
          <span className="ml-auto text-[1rem] text-[#999999]">{question.firstOccurrenceDate}</span>
        </div>
        ...
        {answer && (
          <>
            <div className="text-[14px] pc:text-[1.06rem] text-[#555555]">
              <span className="mr-1 text-[#5C6E56] font-bold">답변</span> {answer.authorName}
            </div>
            <p className="text-[14px] pc:text-[1.06rem] text-[#848484] line-clamp-2 mb-2">
              {answer.content}
            </p>
          </>
        )}
        <div className="flex justify-end items-center text-xs text-[#B4B4B4]">
          <span className="text-[1rem] mr-2">조회수 {question.viewCount}</span>
